...
const AdminUpdateDelete = () => {
  ...
  const {
    ...
    queryKey: ["products"],
    queryFn: () =>
      fetch(/* `${process.env.PUBLIC_URL}/api/products` */ "api/products").then(
        (res) => res.json()
      ),
  });
  ...
  const deleteMutation = useMutation(
    (id) => {
      return fetch(
        /* `${process.env.PUBLIC_URL}/api/products/${id}` */ `/api/products/${id}`,
        {
          method: "DELETE",
          headers: {
              ...
              <button
                ...
                onClick={() => {
                  if (selectedProductId) {
                    ...
                    setModalOpen(false); // Optionally, close the modal after initiating the deletion
                  }
                }}
